...
t3dCategories = []
t3dIndex = 0
t3dCategoryFileModTime = 0

def getT3DCategories(self, context):

    global t3dCategories, t3dCategoryFileModTime, t3dIndex

    catLocation = bpy.path.abspath("//categories")
    if os.path.isfile(catLocation):

        modTime = os.path.getmtime(catLocation)

        if modTime > t3dCategoryFileModTime:
            t3dCategoryFileModTime = modTime
            newEnums = []
            with open(catLocation) as file:
                for enumName in file.readlines():
                    newEnums.append(enumName)

                    # if enumName in t3dCatHas:
                    #     continue

                    # t3dCategories.append((str(t3dIndex), enumName, ""))
                    # t3dCatHas.append(enumName)
                    # t3dIndex += 1

            t3dCategories = [cat for cat in t3dCategories if cat[1] in newEnums]

            for n in newEnums:
                contains = False
                for c in t3dCategories:
                    if c[1] == n:
                        contains = True
                        break

                if not contains:
                    t3dCategories.append((str(t3dIndex), n, ""))
                    t3dIndex += 1

            # for e in t3dCategories:
            #     if not e[1] in newEnums:
            #         t3dCategories.remove(e)

    return t3dCategories
...
class t3dGamePropertyItem__(bpy.types.PropertyGroup):
    ...
    valueCategory : bpy.props.EnumProperty(name = "", description="A category to reference", items=getT3DCategories)
