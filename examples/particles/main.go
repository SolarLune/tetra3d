...
func (g *Game) Init() {
	...
	settings.ScaleAdd.SetAll(0.02)       // Growth is how quickly particles grow in size. If a particle reaches a scale of 0, it will die (unless you disable ParticleSystemSettings.AllowNegativeScale).
	settings.ScaleAdd.Uniform = true
	...
	settings.RotationAdd.SetRangeZ(0.1, 0.1)
	...
	settings.Scale.Uniform = true
	settings.ScaleAdd.SetRanges(0.002, 0.004) // Steadily grow just a little bit, with some randomness
	settings.ScaleAdd.Uniform = true
...
func (g *Game) Update() error {
	...
	g.FireParticleSystem.Update(dt)
