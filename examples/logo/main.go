...
func (g *Game) Init() {
	...
	data, err := tetra3d.LoadGLTFData(bytes.NewReader(logoModel), nil)
...
func (g *Game) Update() error {
	...
	for _, g := range g.Scene.Root.SearchTree().ByPropNameRegex("spin").INodes() {
		g.Rotate(0, 1, 0, 0.05)
	}
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	g.Camera.Render(g.Scene, g.Scene.Root.SearchTree().Lights(), g.Scene.Root.SearchTree().ByName("screen").Models()...)
